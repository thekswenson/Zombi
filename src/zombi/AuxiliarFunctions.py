...
def parse_GFF(gff_file: str, feat_choice, sort=True) -> Tuple[int, List[SeqFeature]]:
    """
    ...
    try:
        for feature in DataIterator(gff_file):
            if feature.featuretype == 'region':
                genome_len = feature.end
            if feature.featuretype == feat_choice:
    ...
    if sort:                    #sort by start index
        genes.sort(key=lambda f: f.start)

    if not genome_len:
        sys.exit(f'No sequence-region found in "{gff_file}"')
    if not genes:
        sys.exit(f'No feature entries found in "{gff_file}".')
...
def read_nucleotide_sequences(fasta: str, genome_folder: str,
    ...
    # Get the frequency of each nucleotide for use in imputation. 
    perc_a = sequence.count("A")/len(sequence)
    perc_t = sequence.count("T")/len(sequence)
    perc_c = sequence.count("C")/len(sequence)
    perc_g = sequence.count("G")/len(sequence)
    percs = [perc_a, perc_t, perc_c, perc_g]

    init_genome_p = Path(genome_folder, initial_genome_info)
    if not init_genome_p.exists():
        raise MissingInfoFileError(init_genome_p)

    # Set up the dictionaries to hold the gene family and intergene sequence SeqRecord objects.
    gidTOseq: Dict[str, SeqRecord] = {}
    didTOseq: Dict[str, SeqRecord] = {}

    # Open the initial genome folder generated by the G mode command. 
    with open(init_genome_p) as f:
        ...
        for line in f:
            ...
            # Check to ensure that the length of the Fasta and GFF files agree. 
            ...
            if type == "GENE_FAMILY":
                temp = sequence[start-1: end]
                # Check to see if there are missing nucleotides. 
                if "N" in temp: # A future version should check for all common placeholders. 
                    seq = ""
                    for i in temp.seq:
                        if i != "N":
                            seq = seq+i
                        else:
                            seq = seq + random.choices(["A","T","C","G"], percs, k=1)[0] # Only works if nucleotides are capitol
                    temp.seq = Seq.Seq(seq)
                    print("Warning: Imputing missing nucleotides (N) in genes using ATCG frequency")
                gidTOseq[did] = temp
            elif type == "DIVISION":
                temp = sequence[start-1: end]
                if "N" in temp:
                    ...
                    for i in temp.seq:
                        if i != "N":
                            ...
                            seq = seq + random.choices(["A","T","C","G"], percs, k=1)[0] 
                    temp.seq = Seq.Seq(seq)
                    print("Warning: Imputing missing nucleotides (N) in intergenes using ATCG frequency")
                didTOseq[did] = temp
            else:
